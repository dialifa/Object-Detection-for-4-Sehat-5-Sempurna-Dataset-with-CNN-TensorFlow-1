...
# model = load_model('model_terbaru.h5')  # Replace with your model path
...
# # Load model
# model = load_model('model_terbaru.h5')
...
# Mengaktifkan pertumbuhan memori GPU untuk mencegah alokasi penuh
...
# Memuat dan mengoptimalkan model untuk inferensi
model = load_model('model_terbaru.h5')
model = tf.function(model)  # Mode eksekusi graph
input_size = (224, 224)  # Sesuaikan dengan ukuran input model Anda

# Label kelas
labels = ['karbohidrat', 'protein', 'buah', 'sayur', 'minuman']

# Inisialisasi video capture
cap = cv2.VideoCapture(0)

# Buffer untuk rata-rata frame
pred_buffer = []
buffer_size = 3

def preprocess_frame(frame):
    # Mengubah ukuran dan memproses frame
...
while True:
    ...
    # Melakukan prediksi
    prediction = model(processed_frame, training=False)
    pred_class = np.argmax(prediction)
    
    # Merata-rata prediksi untuk stabilitas
    ...
    # Menampilkan hasil
    label = labels[smoothed_pred]
    confidence = float(prediction[0][smoothed_pred])
    
    cv2.putText(frame, f'{label} ({confidence:.2f})',
                (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    
    cv2.imshow('Deteksi Makanan', frame)
