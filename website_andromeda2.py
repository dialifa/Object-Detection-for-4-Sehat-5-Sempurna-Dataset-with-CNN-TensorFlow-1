...
def draw_predictions(frame, prediction, confidence):
    """Draw predictions on frame"""
    height, width, _ = frame.shape
    box_start = (int(width * 0.3), int(height * 0.3))
    box_end = (int(width * 0.7), int(height * 0.7))
    ...
    label_position = (box_start[0], box_start[1] - 10)
    cv2.putText(frame, label, label_position, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        ...
        if uploaded_file is not None:
            ...
            if st.button("Detect Food Items"):
                with st.spinner("Processing..."):
                    ...
                    # Preprocess
                    processed_img = preprocess_frame(img_cv)
                    ...
                    if confidence >= CONFIDENCE_THRESHOLD:
                        # Draw predictions
                        result_img = draw_predictions(img_cv, pred_class, confidence)
                        result_img = cv2.cvtColor(result_img, cv2.COLOR_BGR2RGB)
                        
                        # Display results
                        st.image(result_img, caption="Detection Result", use_column_width=True)
                        st.success(f"Detected: {CLASS_NAMES[pred_class]} (Confidence: {confidence:.2f})")
...
# Live Camera page
    ...
    if model is None:
        ...
        while run:
            ...
            if confidence >= CONFIDENCE_THRESHOLD:
                frame = draw_predictions(frame, pred_class, confidence)
